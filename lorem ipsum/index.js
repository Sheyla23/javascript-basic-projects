...
form.addEventListener('submit',(e)=>{
    ...
    if (!number || number < 0 || number>text.length){
      paras = text.slice(0,0)
      section.innerHTML =`<h4> Try again. paragraph length is within the range of 1 - ${text.length} </h4>`;
    } else{
        paras = text.slice(0,number)
        section.innerHTML = paras.map((para)=>{return `<p>${para} </p>`}).join('')
    }
})